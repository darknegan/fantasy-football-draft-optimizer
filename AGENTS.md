...
- **Root `.env` holds Cloudflare deploy credentials (gitignored).** Copy `.env.example` → `.env` and set `CLOUDFLARE_API_TOKEN`. `npm run deploy:web` / `deploy:worker` auto-load it via `scripts/with-root-env.sh`. Account ID is pre-filled in `.env.example`. The same two values must be set as **repository secrets** on GitHub (`CLOUDFLARE_API_TOKEN`, `CLOUDFLARE_ACCOUNT_ID`) for `.github/workflows/deploy-web.yml` to auto-deploy on merge to `main`.
