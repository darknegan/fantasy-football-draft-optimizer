...
- **Root `.env` holds Cloudflare deploy credentials (gitignored).** Copy `.env.example` → `.env` and set `CLOUDFLARE_API_TOKEN`. `npm run deploy:web` / `deploy:worker` auto-load it via `scripts/with-root-env.sh`. Account ID is pre-filled in `.env.example`.
