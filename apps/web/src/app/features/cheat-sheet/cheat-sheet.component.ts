...
@Component({
  ...
  template: `
    <div class="head">
      <div>
        ...
        <p class="dl-muted">
          DraftScore percentiles by position. Targets and avoids carry into the live room.
        </p>
      </div>
    ...
    <div class="grid">
      @for (group of groups(); track group.position) {
        <section class="dl-panel">
          <h2>
            <span class="pos" [class]="group.position">{{ group.position }}</span> tiers
          </h2>
                ...
                @for (p of tier.players; track p.id) {
                  ...
                  >
                    ...
                    <span class="meta dl-mono">
                      ...
                      · {{ p.draftScore }} · {{ p.adpRoundPick }}
                    </span>
                    @if (p.target) {
                      <span class="flag t">T</span>
                    }
                    @if (p.avoid) {
                      <span class="flag a">A</span>
                    }
                  </a>
                }
              </div>
          ...
          @if (group.unranked.length > 0) {
            <div class="tier-block">
              <div class="tier-head">
                <span class="tier unrated">U</span>
                <span class="dl-muted">Unranked — no measured production yet</span>
              </div>
              <div class="players">
                @for (p of group.unranked; track p.id) {
                  <a
                    class="player"
                    [class.target]="p.target"
                    [class.avoid]="p.avoid"
  ...
  styles: `
    ...
    .player.avoid {
      border-color: color-mix(in srgb, var(--dl-grade-red) 45%, transparent);
      opacity: 0.75;
    }
    .name {
      font-weight: 600;
