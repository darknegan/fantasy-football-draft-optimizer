      ...
      <header class="head">
        <div>
          <div class="meta">
            ...
            <span class="dl-muted"
              >{{ p.team }} · age {{ p.age }} · yr {{ p.seasonsInLeague }}</span
            >
          </div>
          <h1>{{ p.name }}</h1>
        </div>
      ...
      @if (evaluation(); as e) {
        <div class="grid">
          <section class="dl-panel">
            ...
            @if (e.ceiling.provisional) {
              <p class="dl-muted">
                RB CeilingScore is provisional until benchmarks land. Showing archetype + risk
                instead.
              </p>
            } @else {
              <div class="factors">
          ...
          <section class="dl-panel side">
            ...
            <div class="kv">
              <span>Archetype</span
              ><strong>{{ e.archetype.archetype.replaceAll('_', ' ') }}</strong>
            </div>
            <div class="kv">
              <span>Archetype EV</span
              ><strong class="dl-mono">{{ e.archetype.archetypeEv.toFixed(3) }}</strong>
            </div>
            <div class="kv">
              <span>Value</span><strong class="dl-mono">{{ e.value.valueScore }}</strong>
            </div>
            <div class="kv">
              <span>ADP</span><strong class="dl-mono">{{ e.value.adpRoundPick }}</strong>
            </div>
            <div class="kv">
              <span>Risk</span><strong class="dl-mono">{{ e.risk.riskProfile }}</strong>
            </div>
            <div class="risk">
            ...
            <div class="kv">
              ...
              ><strong class="dl-mono">{{ (e.ceiling.confidenceScore * 100).toFixed(0) }}%</strong>
            </div>
          </section>
        </div>
      }
